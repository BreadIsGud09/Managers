...
export const listPayments = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await sb.from("tuition_payments").select("*").order("month", { ascending: false }).order("paid_date", { ascending: false });
...
export const upsertPayment = createServerFn({ method: "POST" })
  .validator((d: unknown) => PaymentInput.parse(d))
  .handler(async ({ data }) => {
    const sb = await admin();
    const payload = { ...data, month: normalizeMonth(data.month), note: data.note ?? null };
    if (data.id) {
      const { error } = await sb.from("tuition_payments").update(payload).eq("id", data.id);
      if (error) throw new Error(error.message);
    } else {
      const { id: _i, ...ins } = payload;
      const { error } = await sb.from("tuition_payments").insert(ins);
      if (error) throw new Error(error.message);
    }
    return { ok: true };
  });
...
export const deletePayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const sb = await admin();
    const { error } = await sb.from("tuition_payments").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
